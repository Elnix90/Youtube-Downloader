...
def add_videos(
    ...
    if n==0:
        ...
        if info: print("[Adding videos] No videos to add")
        return
    ...
    for i,video_id in enumerate(videos_to_add):
        ...
        try:
            ...
            if info: fprint(progress_prefix,f"video {video_id} added to {playlist_id}")

        except HttpError as e:
            error_json = e.content.decode() if hasattr(e, 'content') else str(e)  # pyright: ignore[reportUnknownMemberType, reportAttributeAccessIssue, reportUnknownVariableType]
